...
export const api = {
  ...
  addRepo: (url: string) => post<{ job_id: string; repo_url: string; status: string }>("/api/repos", { url }),
  repoJob: (jobId: string) => get<RepoJob>(`/api/repos/jobs/${jobId}`),
  refreshRepo: () => post<{ job_id: string; repo_url: string; status: string }>("/api/repos/refresh", {}),

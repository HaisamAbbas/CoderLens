...
def _initial_state(
    question: str, repo_id: int, max_iterations: int, history: list[dict] | None, simple: bool = False,
) -> InvestigationState:
    return {
        "question": question,
        "repo_id": repo_id,
...
def investigate(
    question: str, repo_id: int, max_iterations: int = 2, history: list[dict] | None = None, simple: bool = False,
) -> dict:
    agent = build_agent()
    state = agent.invoke(_initial_state(question, repo_id, max_iterations, history, simple))
...
def investigate_stream(
    ...
    """
    agent = build_agent()
    initial = _initial_state(question, repo_id, max_iterations, history, simple)
    try:
        seen = 0
        final = None

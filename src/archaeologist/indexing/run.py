...
def extract_to_postgres(repo_id: int | None = None) -> int:
    """Parse every ingested Python file into Symbol rows. Returns symbol count.
    ...
    with session_scope() as session:
        ...
        files = session.scalars(
            select(File).where(File.repo_id == repo.id, File.language == "python",
                               File.content.is_not(None))
        ).all()

        rows: list[dict] = []
        for f in files:
            for sym in symbols.extract_symbols(f.content):
                rows.append(
                    {
                        "repo_id": repo.id,
                        "file_path": f.path,
        ...
        total = len(rows)
        print(f"[1/2] Extracted {total} symbols from {len(files)} Python files.")
    return total
